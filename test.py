...
def test_step(frame, num_frames, model, device, dataset, batch, loss_fn, args, test_losses, test_psnrs):
    ...
    if plot_opt.vnormattn:
        ...
        cb = ax.imshow(vnorm[:, :, pt_idxs[3], :])
        ax.set_title(f'Eval vnorm point {pt_idxs[3]}')

        ax = fig.add_subplot(3, 5, 10)
        cb = ax.imshow(vnorm[:, :, pt_idxs[4], :])
...
def test(model, device, dataset, save_name, args, resume_step, save_dir):
    log_dir = os.path.join("experiments", args.index, save_dir)
...
def main(args, save_name):
    ...
    if args.test.load_path:
        ...
        try:
            resume_step = 250000
            model.load_state_dict(torch.load(os.path.join("experiments", args.index, "model_250000.pth")))
        except:
    ...
    test(model, device, dataset, save_name, args, resume_step, "test")

    # if model.points.shape[0] == 100:
    ...
    #     test(model, device, dataset, save_name, args, resume_step, "test-shovel-z+02")
